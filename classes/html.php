...
class Html
{
    ...
    function getTextField($form, $name, $default='')
    {
        ...
        foreach($this->getForm($form) as $field) {
            ...
            if($field['name'] == $name) {
                ...
                $class = ($temp[count($temp) - 1] != "0,00h" && $field[0]->id == "diffHores2") ? ' text-danger' : '';
                
				if($field[0]->decimal == 'true' && $default != '') $default = number_format((float)$default, 2, ',', '.');
    ...
    */
    function getLabel($form, $name)
    {
        ...
        foreach($this->getForm($form) as $field) {
            /* $default = ($default == '') ?? $field[0]->default; */
            if($field['name'] == $name) {
                $html .= "<label id='".$field[0]->id."'>".$lang->get($field[0]->default)."</label>";
            }
        }
        return $html;
    }
     ...
     * @param $default mixed optional default value
     * @return $html string a complete input field html
     * @see https://www.cssscript.com/autocomplete-typeahead-bootstrap-5/ for typeahead options
    */
    function getTypeaheadField($form, $name, $default='')
    {
        $app    = factory::getApplication();
        $lang   = factory::getLanguage();
        $db     = factory::getDatabase();
        ...
        foreach($this->getForm($form) as $field) {
            ...
            if($field['name'] == $name) {
                ...
                $field[0]->required == 'true' ? $star = " (*)" : $star = "";
                $field[0]->onkeyup != "" ? $onkeyup = " onkeyup='".$field[0]->onkeyup."'" : $onkeyup = "";
                $html .= "<div id='".$id."-field' class='mb-3'>";
                ...
                $html .= "<input type='".$field[0]->type."' id='".$id."' value='".str_replace("'","&#39;",$default)."' name='".$field[0]->name."'";
                $html .= $disabled." ".$onchange." ".$required." ".$onkeyup." ".$readonly." placeholder='".$lang->get($field[0]->placeholder)."' autocomplete='off'";
                $html .= " class='form-control ".$field[0]->classe." ".$classe."'>";
                ...
                $html .= "<div class='position-absolute invisible' id='input_complete'></div>";
                $html .= "<script>";

                //query to database to get the source
                $db->query($field[0]->source);
		        $rows = $db->fetchObjectList();
                foreach($rows as $row) {
                    $result[] = $row->RefTreballFrm;
                }

                $html .= '$(document).ready(function () {';
                $html .= "var datasrc = ['".implode(',', $result)."'];";
                $html .= 'set_autocomplete("'.$id.'", "input_complete", datasrc, start_at_letters=3, count_results=5);';
                $html .= '});';
                $html .= "</script>";
            }
        }
    ...
    function getDateTimeField($form, $name, $default='')
    {
        ...
        foreach($this->getForm($form) as $field) {
            if($field['name'] == $name) {
                ...
                $html .= $disabled." ".$readonly." ".$required." class='form-control ".$field[0]->classe."' autocomplete='off'>";
                $html .= "<span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span>";
                $html .= "</div>";
                $html .= "<div id='".$field[0]->name."Help' class='form-text'>".$lang->get($field[0]->message)."</div><div class='invalid-feedback'>".$lang->get($field[0]->invalid)."</div></div>";
            }
    ...
    */
    public function getRepeatable($form, $fields, $tmpl=null, $list, $value, $key, $footer = null)
    {
        $lang   = factory::getLanguage();

        $first_field = '';

        $html  = "<div class='repeatable' id='".$form."'>";
		$html .= "<table class='wrapper' width='100%'>";
        $html .= '<thead><tr><td colspan="3"><span class="add btn btn-success">'.$lang->get('CW_ADD').'</span>&nbsp;&nbsp;<button type="submit" style="display:none;" class="btn btn-primary save">'.$lang->get('CW_SAVE').'</button></td></tr>';
		$html .= "<tr>";
        $countFields = 0;
        foreach($fields as $field) {
		...
		foreach($fields as $field) {
			foreach($this->getForm($form) as $row) {
                ...
                if($row[0]->type == 'typeahead') { $html .= "<td width='".$width."'>".$this->getTypeaheadField($form, $field)."</td>"; }
		...
		if($tmpl != null) {
			...
			foreach($tmpl as $item) {
				...
				foreach($fields as $field) {
					foreach($this->getForm($form) as $row) {
                        ...
                        if($row[0]->type == 'typeahead') { $html .= "<td class='cell".$i."' width='".$width."'>".$this->getTypeaheadField($form, $field)."</td>"; }
			...
			$html .= "<tr class='fila row0'>";
			
			foreach($fields as $field) {
				foreach($this->getForm($form) as $row) {
					if($row['name'] == $field) {
					$row[0]->width == '' ? $width = '40%' : $width = $row[0]->width;
					$align = $row[0]->align == "right" ? "text-end" : "";

                    if($row[0]->type == 'hidden') { $html .= $this->getTextField($form, $field); }
					if($row[0]->type == 'text') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getTextField($form, $field)."</td>"; }
                    if($row[0]->type == 'typeahead') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getTypeaheadField($form, $field)."</td>"; }
                    if($row[0]->type == 'date') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getDateField($form, $field)."</td>"; }
                    if($row[0]->type == 'datetime') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getDateTimeField($form, $field)."</td>"; }
					if($row[0]->type == 'textarea') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getTextareaField($form, $field)."</td>"; }
					if($row[0]->type == 'list') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getListField($form, $field, "", $list, $value, $key)."</td>"; }
					if($row[0]->type == 'checkbox') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getCheckboxField($form, $field)."</td>"; }
					if($row[0]->type == 'radio') { $html .= "<td class='cell0 ".$align."' width='".$width."'>".$this->getRadioField($form, $field)."</td>"; }
					}
				}
			}

			$html .= '<td width="10%" valign="bottom" align="right"><div class="mb-3"><span class="remove btn btn-danger" data-cell="0"><i class="bi bi-trash-fill"></i></span></div></td>';
		}
		$html .= "<script>";
		$html .= '$(document).ready(function () {';
		...
		$html .= '});';
        $html .= '$(document).on("focus", "#'.$form.' input", function() {';
        $html .= '$("#'.$form.' button.save").css("display", "inline-block");';
        $html .= '});';
		$html .= '});';
		$html .= "</script>";
		$html .= '</tr>';
        ...
        if ($footer != null) {
            ...
            foreach($footer as $field) {
                foreach($this->getForm($form) as $row) {
                    ...
                    $first_field = $row[0]->id;
                    $row[0]->width == '' ? $width = '40%' : $width = $row[0]->width;
                    $row[0]->colspan == '' ? $colspan = '1' : $colspan = $row[0]->colspan;
                    if($row[0]->type == 'hidden') { $html .= $this->getTextField($form, $field['name']); }
                    if($row[0]->type == 'text') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getTextField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'label') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getLabel($form, $field['name'])."</td>"; }
                    if($row[0]->type == 'typeahead') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getTypeaheadField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'date') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getDateField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'datetime') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getDateTimeField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'textarea') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getTextareaField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'list') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getListField($form, $field['name'], $field['value'], $list, $value, $key)."</td>"; }
                    if($row[0]->type == 'checkbox') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getCheckboxField($form, $field['name'], $field['value'])."</td>"; }
                    if($row[0]->type == 'radio') { $html .= "<td colspan='".$colspan."' width='".$width."'>".$this->getRadioField($form, $field['name'], $field['value'])."</td>"; }
                    }
                }
            }
        ...
        foreach($this->getForm($form) as $field) {
            if($field['name'] == $name) {
                ...
                foreach($field[0]->option as $option) {
                    ...
                    $option['bgcolor'] != '' ? $styles = "style='background-color:".$option['bgcolor'].";color:".$option['color']."'" : $styles = "";
                    $html .= "<option value='".$option['value']."' $styles $click $selected>".$lang->get($option[0])."</option>";
                }
